...
function SideBar({ clickedChat }) {
  ...
  return (
    <div className="p-8 px-4 bg-gray-50  hidden md:block md:w-1/4 lg:w-1/5" style={{ minWidth: "260px" }}>
      ...
      >
        <div className="ml-3 flex pb-4">
          ...
          <div
            className={`w-14 h-7 flex items-center rounded-full p-1 cursor-pointer ${enabled ? "bg-blue-900" : "bg-gray-300"}`}
            onClick={() => setEnabled(!enabled)}
          >
            <div
              className={`bg-white w-5 h-5 rounded-full shadow-md transform transition-transform ${enabled ? "translate-x-7" : "translate-x-0"}`}
            ></div>
          </div>
        </div>
